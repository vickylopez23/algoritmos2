...
int main(int argc, char *argv[]) {
    ...
    int min_temp = temp_min (array);

    printf ("The temperatura minim in Cordoba in his history is: %d\n",min_temp);
    printf ("\n");

    int out[YEARS];

    temp_max_year(array,out);
    printf ("The maxim temperature per year in Cordoba: \n");
    printf ("[");
    ...
    month_max_rainfall(array,outt);
    printf ("The months with more rainfalls per each year: \n");
    printf ("[");
    for (unsigned int i=0; i<YEARS; ++i){
        printf ("%d,",outt[i]);
    }
